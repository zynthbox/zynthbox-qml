...
class zynthian_gui_osc_browser(zynthian_gui_selector):
	...
	def get_osc_paths(self, path=''):
		self.list_data=[]
		if path=='root':
			self.osc_path="/part"+str(zynqtgui.curlayer.get_midi_chan())+"/"
		else:
			self.osc_path=self.osc_path+path
		liblo.send(zynqtgui.osc_target, "/path-search",self.osc_path,"")
	...
	def select_action(self, i):
		...
		if tnode=='dir':
			...
			if len(parts)>1:
				title=parts[-2]+" "+title
			zynqtgui.screens['control'].zcontrollers_config[2]=(title[:-2],self.osc_path+path,64,127) #TODO
			liblo.send(zynqtgui.osc_target, self.osc_path+path)
			zynqtgui.show_screen('control')
		elif tnode=='bool':
			#TODO: Toogle the value!!
			liblo.send(zynqtgui.osc_target, self.osc_path+path,True)
			zynqtgui.show_screen('control')

	def set_select_path(self):
		self.select_path = (zynqtgui.zyngine.get_presetpath())
