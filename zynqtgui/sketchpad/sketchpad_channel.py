...
class sketchpad_channel(QObject):
        ...
        if not self.__song__.__to_be_deleted__:
            for index, midi_channel in enumerate(self.chainedSounds):
                ...
                if midi_channel >= 0 and self.checkIfLayerExists(midi_channel):
                    ...
                    if layer.engine.version_info.cutoffControls is not None:
                        for controllerName in layer.engine.version_info.cutoffControls:
                            if controllerName in synth_controllers_dict:
                                self.__filter_cutoff_controllers[index].add_control(synth_controllers_dict[controllerName])
                    ...
                    if layer.engine.version_info.resonanceControls is not None:
                        for controllerName in layer.engine.version_info.resonanceControls:
                            if controllerName in synth_controllers_dict:
                                self.__filter_resonance_controllers[index].add_control(synth_controllers_dict[controllerName])
            ...
            for index, layer in enumerate(self.chainedFx):
                ...
                if layer is not None:
                    ...
                    if layer.engine.version_info.cutoffControls is not None:
                        for controllerName in layer.engine.version_info.cutoffControls:
                            if controllerName in synth_controllers_dict:
                                self.__fx_filter_cutoff_controllers[index].add_control(synth_controllers_dict[controllerName])
                    elif "cutoff" in synth_controllers_dict:
                    ...
                    if layer.engine.version_info.resonanceControls is not None:
                        for controllerName in layer.engine.version_info.resonanceControls:
                            if controllerName in synth_controllers_dict:
                                self.__fx_filter_resonance_controllers[index].add_control(synth_controllers_dict[controllerName])
                    elif "resonance" in synth_controllers_dict:
            ...
            for index, layer in enumerate(self.chainedSketchFx):
                ...
                if layer is not None:
                    ...
                    if layer.engine.version_info.cutoffControls is not None:
                        for controllerName in layer.engine.version_info.cutoffControls:
                            if controllerName in synth_controllers_dict:
                                self.__sketchfx_filter_cutoff_controllers[index].add_control(synth_controllers_dict[controllerName])
                    elif "cutoff" in synth_controllers_dict:
                    ...
                    if layer.engine.version_info.resonanceControls is not None:
                        for controllerName in layer.engine.version_info.resonanceControls:
                            if controllerName in synth_controllers_dict:
                                self.__sketchfx_filter_resonance_controllers[index].add_control(synth_controllers_dict[controllerName])
                    elif "resonance" in synth_controllers_dict:
    ...
    @Slot(int)
    def selectPreviousSynthBank(self, slot_index):
        midi_channel = self.chainedSounds[slot_index]

        if midi_channel >= 0 and self.checkIfLayerExists(midi_channel):
            layer = self.zynqtgui.layer.layer_midi_map[midi_channel]
            if layer.bank_index > 0:
                prev_volume = None
                try:
                    prev_volume = self.zynqtgui.fixed_layers.volumeControllers[midi_channel].value
                except Exception: pass

                layer.set_bank(layer.bank_index - 1)
                layer.set_preset(0)
                self.zynqtgui.fixed_layers.update_mixers()
                ...
                self.zynqtgui.bank.select(layer.bank_index)
                self.zynqtgui.preset.select(0)
                self.zynqtgui.layer.emit_layer_preset_changed(layer)
                self.zynqtgui.screens['control'].show()
                self.zynqtgui.layer.fill_list()
                self.zynqtgui.screens['snapshot'].schedule_save_last_state_snapshot()
    ...
    @Slot(int)
    def selectNextSynthBank(self, slot_index):
        midi_channel = self.chainedSounds[slot_index]

        if midi_channel >= 0 and self.checkIfLayerExists(midi_channel):
            layer = self.zynqtgui.layer.layer_midi_map[midi_channel]
            if layer.bank_index < len(layer.bank_list) - 1:
                prev_volume = None
                try:
                    prev_volume = self.zynqtgui.fixed_layers.volumeControllers[midi_channel].value
                except Exception: pass
    ...
    @Slot(int)
    def selectPreviousFxBank(self, slot_index):
        layer = self.chainedFx[slot_index]

        if layer is not None and layer.bank_index > 0:
            layer.set_bank(layer.bank_index - 1)
            layer.set_preset(0)
            self.zynqtgui.bank.select(layer.bank_index)
            self.zynqtgui.preset.select(0)
    ...
    @Slot(int)
    def selectNextFxBank(self, slot_index):
        layer = self.chainedFx[slot_index]

        if layer is not None and layer.bank_index < len(layer.bank_list) - 1:
            layer.set_bank(layer.bank_index - 1)
            layer.set_preset(0)
            self.zynqtgui.bank.select(layer.bank_index)
            self.zynqtgui.preset.select(0)
