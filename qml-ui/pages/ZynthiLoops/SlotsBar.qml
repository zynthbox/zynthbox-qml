    ...
    function selectConnectedSound() {
        if (root.selectedSlotRowItem.track.connectedSound >= 0) {
            ...
            if (root.selectedSlotRowItem.track.connectedPattern >= 0) {
                var pattern = ZynQuick.PlayGridManager.getSequenceModel("Scene "+zynthian.zynthiloops.song.scenesModel.selectedSceneName).getByPart(root.selectedSlotRowItem.track.id, root.selectedSlotRowItem.track.selectedPart);
