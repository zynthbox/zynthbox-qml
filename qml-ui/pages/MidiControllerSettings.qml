...
ZUI.ScreenPage {
            ...
            ListView {
                ...
                delegate: QQC2.ItemDelegate {
                    ...
                    contentItem: RowLayout {
                        ...
                        Kirigami.Icon {
                            // The device has both input and output
                            // The device has only input
                            // The device has only output

                            Layout.fillHeight: true
                            Layout.margins: Kirigami.Units.largeSpacing
                            source: model.hasInput ? model.hasOutput ? "escape-direction-horizontal" : "escape-direction-left" : model.hasOutput ? "escape-direction-right" : "escape-direction-vertical" // This one's unlikely to happen, as a midi device with neither input nor output is a bit... not right, but hey, fill it up, why not, might be useful for some debuggery
                        }
    ...
    ZUI.ComboBox {
        ...
        visible: false
        textRole: slotType === 0 ? "clipSlotText" : slotType === 1 ? "soundSlotText" : slotType === 2 ? "fxSlotText" : "text"
        onActivated: function(activatedIndex) {
        ...
        model: ListModel {
            ListElement {
                text: "Current Slot"
                clipSlotText: "Current Clip"
                soundSlotText: "Current Sound Slot"
                fxSlotText: "Current FX Slot"
                value: -1
            }
            ...
            ListElement {
                text: "Slot 1"
                clipSlotText: "Clip 1"
                soundSlotText: "Sound Slot 1"
                fxSlotText: "FX Slot 1"
                value: 0
            }
            ...
            ListElement {
                text: "Slot 2"
                clipSlotText: "Clip 2"
                soundSlotText: "Sound Slot 2"
                fxSlotText: "FX Slot 2"
                value: 1
            }
            ...
            ListElement {
                text: "Slot 3"
                clipSlotText: "Clip 3"
                soundSlotText: "Sound Slot 3"
                fxSlotText: "FX Slot 3"
                value: 2
            }
            ...
            ListElement {
                text: "Slot 4"
                clipSlotText: "Clip 4"
                soundSlotText: "Sound Slot 4"
                fxSlotText: "FX Slot 4"
                value: 3
            }
            ...
            ListElement {
                text: "Slot 5"
                clipSlotText: "Clip 5"
                soundSlotText: "Sound Slot 5"
                fxSlotText: "FX Slot 5"
                value: 4
            }
    ...
    ZUI.ComboBox {
        ...
        model: ListModel {
            ...
            ListElement {
                text: "MIDI Channel 10"
                value: 9
            }

            ListElement {
                text: "MIDI Channel 11"
                value: 10
            }

            ListElement {
                text: "MIDI Channel 12"
                value: 11
            }

            ListElement {
                text: "MIDI Channel 13"
                value: 12
            }

            ListElement {
                text: "MIDI Channel 14"
                value: 13
            }

            ListElement {
                text: "MIDI Channel 15"
                value: 14
            }

            ListElement {
                text: "MIDI Channel 16"
                value: 15
            }
    ...
    ZUI.ComboBox {
        ...
        Component.onCompleted: {
            for (let eventIndex = 0; eventIndex < 142; ++eventIndex) {
                model.append({
                    "text": Zynthbox.CUIAHelper.cuiaTitle(eventIndex),
                    "value": eventIndex
                });
            }
        }
    ...
    ZUI.ComboBox {
        ...
        model: ListModel {
            ListElement {
                text: "Matched MIDI message's Byte 1 value"
                value: -1
            }

            ListElement {
                text: "Matched MIDI message's Byte 2 value (where available, otherwise 0)"
                value: -2
            }

            ListElement {
                text: "Matched MIDI message's Byte 3 value (where available, otherwise 0)"
                value: -3
            }

            ListElement {
                text: "Matched MIDI message's event channel (where available, otherwise 0)"
                value: -4
            }
                ...
                delegate: RowLayout {
                    ...
                    Item {
                        ...
                        MouseArea {
                            anchors.fill: parent
                            onClicked: {
                                midiEventListener.selectedIndex = model.index;
                            }
                        }
                    ...
                    QQC2.Label {
                        Layout.fillWidth: true
                        text: modelData.length === 1 ? applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0]) : modelData.length === 2 ? "%1 with value %2".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(modelData[1]) : modelData.length === 3 ? 127 < modelData[0] && modelData[0] < 160 ? "%1 with note %2 and velocity %3".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(Zynthbox.KeyScales.midiNoteName(modelData[1])).arg(modelData[2]) : 159 < modelData[0] && modelData[0] < 176 ? "%1 for note %2 with pressure %3".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(Zynthbox.KeyScales.midiNoteName(modelData[1])).arg(modelData[2]) : 175 < modelData[0] && modelData[0] < 192 ? "%1 with function %2 and value %3".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(applicationWindow().midiBytePicker.byteValueToCCName(modelData[1])).arg(modelData[2]) : 191 < modelData[0] && modelData[0] < 208 ? "%1 with program %2".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(modelData[1]) : 207 < modelData[0] && modelData[0] < 224 ? "%1 with pressure %2".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(modelData[1]) : "%1 with values %2 and %3".arg(applicationWindow().midiBytePicker.byteValueToMessageName(modelData[0])).arg(modelData[1]).arg(modelData[2]) : "(weird message with a length that isn't between 1 and 3" + modelData + ")"
