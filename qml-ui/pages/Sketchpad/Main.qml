...
//    property var lastSelectedObj: null
    ...
    */
    ...
    function updateSelectedSketchGain(sign, slot=-1) {
        ...
        function valueSetter(value) {
            if (clip != null && !clip.isEmpty) {
                clip.metadata.gain = Zynthian.CommonUtils.clamp(value, -100, 24)
                applicationWindow().showOsd({
                    ...
                    step: 1,
                    defaultValue: 0,
                    currentValue: parseInt(clip.metadata.gain),
                    startLabel: "-100 dB",
                    stopLabel: "24 dB",
                    valueLabel: qsTr("%1 dB").arg(clip.metadata.gain),
                    setValueFunction: valueSetter,
                    showValueLabel: true,
                    showResetToDefault: true,
                    showVisualZero: true
        ...
        valueSetter(clip.metadata.gain + sign)
    }
    /**
    ...
    function updateClipGain(clip, sign) {
        if (clip != null) {
            clip.metadata.gain = Zynthian.CommonUtils.clamp(clip.metadata.gain + sign, -100, 24)
        }
    }
    /**
     * Update clip pitch
